...
uint32_t qlen_dump_interval = 100000000, qlen_mon_interval = 1000000, sketch_mon_interval = 1000000, acc_mon_interval = 500000;
uint64_t qlen_mon_start = 2000000000, qlen_mon_end = 2100000000, acc_mon_start = 2001000000;
uint64_t parameter_tuning_start = 2002000000, parameter_tuning_end = 2500000000, parameter_tuning_interval = 1002000;
...
std::map<std::string, std::string> readParameterText(const std::string& filename)
{
...
// monitor metric when tuning
void monitor_buffer_new(FILE* qlen_output, NodeContainer *n){
	// NIC RTT
	std::string m_rtt_path = "mix/rtt.tr";
	std::ofstream m_rtt_file;
	m_rtt_file.open(m_rtt_path, std::ios::out);

	for (uint32_t i = 0; i < n->GetN(); i++) {
		if (n->Get(i)->GetNodeType() == 0) { // is NIC
			std::map<std::string, int> nic_map_rtt = n->Get(i)->GetObject<RdmaDriver>()->m_rdma->m_map_last_rtt;
			for (auto it : nic_map_rtt)
			{
				m_rtt_file << Simulator::Now().GetSeconds() << " " << it.first << " " << it.second << "\n";
			}
			n->Get(i)->GetObject<RdmaDriver>()->m_rdma->ClearRttMap();
		}
	}
	m_rtt_file.close();

	// switch throughput
	std::string m_switch_portrate_path = "mix/switch_portrate.tr";
	std::ofstream m_switch_portrate_file;
	m_switch_portrate_file.open(m_switch_portrate_path, std::ios::out);

	for (uint32_t i = 0; i < n->GetN(); i++){
		if (n->Get(i)->GetNodeType() == 1){     // is switch
			Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n->Get(i));
			uint64_t* rxBytes = sw->getRxBytes();
			for (int i = 0; i < 257; i++)
			{
				if (rxBytes[i] != 0)
				{
					m_switch_portrate_file << Simulator::Now().GetSeconds() << " " << sw->GetId() << " " << i << " " << rxBytes[i] << "\n";
					rxBytes[i] = 0;
				}
			}
		}
	}
	m_switch_portrate_file.close();

	// switch queue length snapshot
	if (qlen_output != nullptr){
		for (uint32_t i = 0; i < n->GetN(); i++){
			...
			for (uint32_t port = 1; port < sw->GetNDevices(); port++){
				uint32_t qlen_bytes = 0;
				for (uint32_t q = 0; q < SwitchMmu::qCnt; q++){
					qlen_bytes += sw->m_mmu->egress_bytes[port][q];
				}
				if (qlen_bytes == 0)
					continue;
				fprintf(qlen_output, "%f %u %u %u\n",
				        Simulator::Now().GetSeconds(), sw->GetId(), port, qlen_bytes);
			}
		}
		fflush(qlen_output);
	}
	...
	if (Simulator::Now().GetTimeStep() < qlen_mon_end)
		Simulator::Schedule(NanoSeconds(qlen_mon_interval), &monitor_buffer_new, qlen_output, n);
}

// upload sketch
void monitor_sketch(NodeContainer *n)
{
	std::string m_switch_sketch_heavypart_path = "mix/switch_sketch_heavypart.tr";
	std::ofstream m_switch_sketch_heavypart_file;
	m_switch_sketch_heavypart_file.open(m_switch_sketch_heavypart_path, std::ios::out);
	...
	for (uint32_t i = 0; i < n->GetN(); i++){
		if (n->Get(i)->GetNodeType() == 1){     // is switch
			...
			if (sw->add_to_sketch)
			{
				vector<double> lightpart_dist;
				vector<pair<uint32_t, int>> heavypart_vector;
				sw->reportSketch(lightpart_dist, heavypart_vector);

				m_switch_sketch_heavypart_file << Simulator::Now().GetSeconds() << " " << sw->GetId() << " ";
				for (int j = 0; j < heavypart_vector.size(); j++)
				{
					auto item = heavypart_vector[j];
					m_switch_sketch_heavypart_file << item.first << "-" << item.second << " ";
				}
				m_switch_sketch_heavypart_file << "\n";
			}
		}
	}

	m_switch_sketch_heavypart_file.close();
	
	if (Simulator::Now().GetTimeStep() < qlen_mon_end)
		Simulator::Schedule(NanoSeconds(sketch_mon_interval), &monitor_sketch, n);
}

void parameter_tuning(NodeContainer *n)
{
	std::cout << Simulator::Now().GetSeconds() << " parameter_tuning\n";
	std::string parameter_tuning_file = "mix/parameter.txt";
	std::map<std::string, std::string> parameter_map = readParameterText(parameter_tuning_file);
	for (uint32_t i = 0; i < n->GetN(); i++) {
		if (n->Get(i)->GetNodeType() == 0) { // is NIC
			n->Get(i)->GetObject<RdmaDriver>()->m_rdma->ChangeParameters(parameter_map);
		}
		else if (n->Get(i)->GetNodeType() == 1) {  // is switch
			n->Get(i)->GetObject<SwitchNode>()->ChangeECNthreshold(parameter_map);
		}
	}
	if (Simulator::Now().GetTimeStep() < parameter_tuning_end)
		Simulator::Schedule(NanoSeconds(parameter_tuning_interval), &parameter_tuning, n);
}
...
void delete_files()
{
	std::string nic_datarate_path = "mix/nic_datarate.tr";
	std::string switch_portrate_path = "mix/switch_portrate.tr";
	std::string packet_path = "mix/packet_path.tr";
	std::string cnp_path = "mix/nic_cnp_received.tr";
	std::string rtt_path = "mix/rtt.tr";
	std::string sketch_path = "mix/switch_sketch.tr";

	if (remove(switch_portrate_path.c_str()))
	{
		std::cout << "delete original switch_portrate.tr!\n";
	}

	if (remove(nic_datarate_path.c_str()))
	{
		std::cout << "delete original nic_datarate.tr!\n";
	}

	if (remove(packet_path.c_str()))
	{
		std::cout << "delete original packet_path.tr!\n";
	}

	if (remove(cnp_path.c_str()))
	{
		std::cout << "delete original nic_cnp_received.tr!\n";
	}

	if (remove(rtt_path.c_str()))
	{
		std::cout << "delete original rtt.tr!\n";
	}

	// if (remove(sketch_path.c_str()))
	// {
	// 	std::cout << "delete original sketch.tr!\n";
	// }
}
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			}else if (key.compare("PARAMETER_TUNING_START") == 0){
				conf >> parameter_tuning_start;
				std::cout << "PARAMETER_TUNING_START\t\t\t\t" << parameter_tuning_start << '\n';
			}else if (key.compare("PARAMETER_TUNING_END") == 0){
				conf >> parameter_tuning_end;
				std::cout << "PARAMETER_TUNING_END\t\t\t\t" << parameter_tuning_end << '\n';
	...
	if (cc_mode == 7 || cc_mode == 1) // timely & dcqcn, use ts
		IntHeader::mode = IntHeader::TS;
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 1){ // is switch
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++){
				...
				//std::cout << "j=" << j << ", rate2kmin[rate]=" << rate2kmin[rate] << "\n";
				// set pfc
	...
	Simulator::Schedule(NanoSeconds(qlen_mon_start), &monitor_buffer_new, qlen_output, &n);
	Simulator::Schedule(NanoSeconds(qlen_mon_start), &monitor_sketch, &n);
	Simulator::Schedule(NanoSeconds(parameter_tuning_start), &parameter_tuning, &n);
