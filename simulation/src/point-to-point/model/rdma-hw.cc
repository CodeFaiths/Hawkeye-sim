...
namespace {
std::string
Ipv4AddressToString (const ns3::Ipv4Address &ip)
{
...
uint64_t RdmaHw::GetQpKey(uint32_t dip, uint16_t sport, uint16_t pg){
	return ((uint64_t)dip << 32) | ((uint64_t)sport << 16) | (uint64_t)pg; // 将目的IP、源端口、优先级拼装成唯一键
}
Ptr<RdmaQueuePair> RdmaHw::GetQp(uint32_t dip, uint16_t sport, uint16_t pg){
	uint64_t key = GetQpKey(dip, sport, pg); // 生成查找键
	auto it = m_qpMap.find(key); // 在QP映射表中查找
	if (it != m_qpMap.end())
		return it->second; // 找到则返回该QP指针
	return NULL; // 未找到返回空指针
}

//size:字节数 pg:优先级 sip:源IP dip:目的IP sport:源端口 dport:目的端口 win:窗口大小 baseRtt:基础RTT notifyAppFinish:应用完成回调
...
void RdmaHw::QpComplete(Ptr<RdmaQueuePair> qp){
	...
	Time fct = Simulator::Now() - qp->startTime;
	uint32_t nodeIdComplete = m_node ? m_node->GetId() : std::numeric_limits<uint32_t>::max();
	std::cout << "[RdmaHw Complete t=" << Simulator::Now().GetSeconds() << "s] "
			  << "node=" << nodeIdComplete
			  << " src=" << Ipv4AddressToString(Ipv4Address(qp->sip)) << ":" << qp->sport
			  << " -> dst=" << Ipv4AddressToString(Ipv4Address(qp->dip)) << ":" << qp->dport
			  << " size=" << qp->m_size << "B"
			  << " FCT=" << fct.GetSeconds() << "s"
			  << std::endl;
