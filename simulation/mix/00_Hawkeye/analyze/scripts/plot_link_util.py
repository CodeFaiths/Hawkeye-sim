...
def main() -> None:
    ...
    parser.add_argument("--topology", type=Path, default=base_dir / "config" / "topo_incast_5to1.txt", help="Topology file that lists the switch node IDs on the second line.")
    ...
    def canonical_label(lbl: str) -> str:
        """Normalize label to SW{node}-P{port} or H{node}-P{port} format."""
        s = lbl.strip().upper()
        if s.startswith("SW"):
            body = s[2:].replace("-P", "-")
            parts = body.split("-")
